...
from app.web.status.status_update import (
    ProgrammingExerciseChatStatusCallback,
    CourseChatStatusCallback,
    CompetencyExtractionCallback,
)
from app.pipeline.chat.course_chat_pipeline import CourseChatPipeline
from app.pipeline.chat.programming_exercise_chat_pipeline import (
    ProgrammingExerciseChatPipeline,
)
...
def run_exercise_chat_pipeline_worker(dto: ProgrammingExerciseChatPipelineExecutionDTO):
    try:
        ...
        pipeline = ProgrammingExerciseChatPipeline(callback=callback)
...
@router.post(
    "/programming-exercise-chat/{variant}/run",
    status_code=status.HTTP_202_ACCEPTED,
    dependencies=[Depends(TokenValidator())],
)
def run_exercise_chat_pipeline(
    variant: str, dto: ProgrammingExerciseChatPipelineExecutionDTO
):
